...
def loginPage(logdata):
    ...
    heading = Label(login_frame,text="Quiz Game Login",fg="black",bg="white")
    ...
    log.configure(width = 15,height=1, activebackground = "#33B5E5", bg ="green" ,relief = FLAT)
...
def signUpPage():
    ...
    heading = Label(sup_frame,text="Quiz Game SignUp",fg="black",bg="white")
    heading.config(font=('calibri 40'))
    heading.place(relx=0.2,rely=0.1)
...
def menu1(i):
    ...
    wel = Label(menu_canvas,text=' W E L C O M E  T O  Q U I Z  G A M E ',fg="white",bg="#101357") 
...
def difficult():
    ...
    hardQ = [
                [
       "All keywords in Python are in ___",
        "lower case",
        "UPPER CASE",
        "Capitalized",
        "None of the mentioned"
...
def showMark(mark):
    ...
    mlabel = Label(show_canvas,text=st,fg="green")
    mlabel.config(font=('Helvetica 25'))
    mlabel.place(relx=0.5,rely=0.5,anchor=CENTER)
    exit1 = Button(show_frame,command=exit,text='REPLAY')
    exit1.configure(width = 8,height=2, activebackground = "#33B5E5", bg ='skyblue', relief = RAISED)
    exit1.place(relx=0.8,rely=0.82,anchor=CENTER)
    exit = Button(show_frame,command=exit2,text='EXIT')
    exit.configure(width = 5,height=2, activebackground = "#33B5E5", bg ='red', relief = RAISED)
    exit.place(relx=0.05,rely=0.82,anchor=CENTER)
